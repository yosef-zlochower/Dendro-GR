...
void readParamTOMLFile(const char* fName, MPI_Comm comm) {
    ...
    if (parFile.contains("BSSN_SIS_TO_CONSTRAINT_WAMR_TRANSITION_TIME"))
        bssn::BSSN_SIS_TO_CONSTRAINT_WAMR_TRANSITION_TIME =
            parFile["BSSN_SIS_TO_CONSTRAINT_WAMR_TRANSITION_TIME"].as_floating();

    if (parFile.contains("BSSN_INNER_SIS_REGION_OUTER_BOUND"))
        bssn::BSSN_INNER_SIS_REGION_OUTER_BOUND =
            parFile["BSSN_INNER_SIS_REGION_OUTER_BOUND"].as_floating();
    ...
    if (parFile.contains("BSSN_BOX_RADII_1")) {
        for (unsigned int i = 0; i < bssn::BSSN_BOX_NUM_LEVELS[0]; i++)
            bssn::BSSN_BOX_RADII_1[i] =
                parFile["BSSN_BOX_RADII_1"][i].as_floating();
    } else if (bssn::BSSN_REFINEMENT_MODE == SPHERE_IN_SPHERE) {
    ...
    if (parFile.contains("BSSN_BOX_RADII_2")) {
        for (unsigned int i = 0; i < bssn::BSSN_BOX_NUM_LEVELS[1]; i++)
            bssn::BSSN_BOX_RADII_2[i] =
                parFile["BSSN_BOX_RADII_2"][i].as_floating();
    } else if (bssn::BSSN_REFINEMENT_MODE == SPHERE_IN_SPHERE) {

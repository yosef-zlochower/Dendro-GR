    ...
    if (ts_mode == 1) {
		...
		bssnCtx->calc_constraints();
            ...
            if ((step % bssn::BSSN_REMESH_TEST_FREQ) == 0) {
                ...
                if (!(ets->get_global_rank())) {
                    std::cout
                        << "current simulation time and run speed (M / hour) : "
                        << ets->curr_time() << " "
                        << (ets->curr_time() - sim_interval_start) /
                               (current_wall_time - wall_interval_start) * 3600
                        << std::endl;
                }
        ...
        if (!(ets->get_global_rank())) {
            const double current_time = MPI_Wtime();
            std::cout << " ETS total evolution time (hours) : "
                      << (current_time - init_done_time) / 3600 << std::endl;
            std::cout << " ETS average run speed (M / hour) : "
                      << (ets->curr_time() - init_time) /
                             (current_time - init_done_time) * 3600
                      << std::endl;
        }

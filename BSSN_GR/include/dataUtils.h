...
// OCT_IGNORE is used by dataUtils.cpp functions to coordinate which function
// is setting the refinement flag at a particular point. It should never be
// passed to dendrolib functions.
...
/**
 * @brief Sphere-in-Sphere refinement
 */
bool isRemeshSinS(ot::Mesh* pMesh, const Point* bhLoc);
std::vector<unsigned int> isRemeshSinSHelper(ot::Mesh* pMesh,
                                             const Point* bhLoc);
std::vector<unsigned int> isRemeshSinSInitHelper(ot::Mesh* pMesh,
                                                  const Point* bhLoc);

/**
 * @brief SiS + WAMR combination refinement
 */
bool isRemeshSiSCombination(
    ot::Mesh* pMesh, const Point* bhLoc, const double** unzippedVec,
    const unsigned int* varIds, const unsigned int numVars,
    std::function<double(double, double, double, double*)> wavelet_tol,
    double amr_coarse_fac);

/**
 * @brief Constraint-based refinement
 */
...
/**
 * @brief Constraint-based WAMR refinement
 */
...
/**
 * @brief WAMR helper function
 */
std::vector<unsigned int> isReMeshWAMRHelper(
    ot::Mesh* pMesh, const double** unzippedVec, const unsigned int* varIds,
    const unsigned int numVars,
    std::function<double(double, double, double, double*)> wavelet_tol,
    double amr_coarse_fac);
